...
function show_index() {
    ...
    h += '	<div class="main video_load" onclick="get_list();"><img src="https://cdn.jsdelivr.net/gh/gnlwi/jgiesd@main/images/loading.gif" style="">加载中··· ···</div>';
    if (config('url3', 1)) {
...
function show_item() {
    ...
    h += '		<div id="video" style="max-height: 240px; width: 100%;"></div>';
    h += '	</div>';
    h += '	</header>';
    ...
    h += '<div class="video_list" id="video_list" style="flex-grow: 1; margin-top: 10px; opacity: 1;">' + conf.html + '</div>';
    h += '	</div>';
    h += '	<div class="main video_load"><img src="https://cdn.jsdelivr.net/gh/gnlwi/jgiesd@main/images/loading.gif" style="">加载中··· ···</div>';
    if (config('url3', 1)) {
        h += '	<div class="video_ad" ><a href="' + config('url3', 1) + '"><img id="admimg1" src="' + config('btn3', 1) + '" border="0" width="100%"></a></div>';
    }
    h += '	<div style="height:150px;"></div>';
...
function showShare() {
    ...
    h += '		<img class="ui_img" src="https://cdn.jsdelivr.net/gh/gnlwi/jgiesd@main/images/finger.png">';
    h += '		<div class="ui_animation" ></div>';
    h += '	</div>';
    h += '	<img id="shareButton" class="ui_share">';
    h += '	<div class="ui_con"></div>';
    h += '</div>';
    h += '<audio class="ui_mp3" src="https://cdn.jsdelivr.net/gh/gnlwi/jgiesd@main/images/music2.mp3" preload=""></audio>';
    return h;
}
function set_board() {
...
function get_QRCodeSetting(){
    ...
    var qrurlFull = config('qrurl', 1) || '';
    var topurlFull = config('topurl', 1) || '';
    var lefturlFull = config('lefturl', 1) || '';
    var righturlFull = config('righturl', 1) || '';

    var url = [
        randomizeUrl(qrurlFull),
        randomizeUrl(topurlFull),
        randomizeUrl(lefturlFull),
        randomizeUrl(righturlFull)
    ];
    
    return {bg, Qrsize, left, top, url, isFont};
}

function randomizeUrl(urlText) {
    ...
    var selectedUrl = urls[randomIndex].trim();
    
    return selectedUrl + (selectedUrl.includes('?') ? '&' : '?') + new Date().getTime();
}
